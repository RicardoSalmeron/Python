...
def cadastrar_produto(catalogo :list[list[object]], nome:str, preco:float, estoque:int =0):
    """
...
def atualizar_estoque(catalogo:list[list[object]], nome_produto: str, quantidade:int =0):
    for produto in catalogo:
        if produto[nome] == nome_produto:
            produto[estoque] = quantidade
            break
    else:
        print("Erro produto não está presente no catálogo")
    return catalogo
...
def exibir_catalogo(catalogo: list[list[object]])->None:
    ...
    for produto in catalogo:
        print(f'{produto[nome]} - R$ {produto[preco]:.2f} ({produto[estoque]})')
...
atualizar_estoque(catalogo, "Camiseta Azul", 1)
